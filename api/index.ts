...
// 2F ÔÇö Add CORS Restriction: Guard against CSRF & arbitrary cross-origin script executions
...
// 2E ÔÇö Add Request Body Size Limit: Guard against buffer overflows and DoS exhaustion
app.use(express.json({ limit: "500kb" }));
app.use(express.urlencoded({ extended: true, limit: "500kb" }));

// Initialize Supabase Server Client (with env var validation)
...
let sbAdminClient: any = null;
try {
  sbClient = createClient(supabaseUrl || "", supabaseAnonKey || "");
  supabase = sbClient;
  sbAdminClient = createClient(supabaseUrl || "", supabaseServiceKey || "");
} catch (e: any) {
  console.error("Supabase client init failed (env vars missing?):", e?.message);
}
...
async function initializeDatabase() {
  ...
  if (!supabaseUrl || !serviceKey) {
    console.warn("ÔÜá´©Å DB Setup skipped: Missing Supabase credentials");
    return;
  }

  const logs: string[] = [];
  try {
    console.log("­ƒöº CARD MRI: Running database self-healing setup...");
    const result = await runDatabaseSetup(supabaseUrl, serviceKey, logs);
    if (result.success) {
      console.log("Ô£à CARD MRI Database: All tables verified and ready");
    } else {
      console.warn("ÔÜá´©Å CARD MRI Database: Some setup steps had issues:", logs);
    }
  } catch (e: any) {
    console.warn("ÔÜá´©Å DB Setup error (non-fatal):", e.message);
  }
  dbSetupCompleted = true;
}

// Call initializeDatabase() immediately after the sbClient is created, before any route definitions
try { initializeDatabase().catch(console.warn); } catch (e: any) { console.warn("DB setup init call failed:", e?.message); }
...
// 2A ÔÇö Hash Passwords with bcrypt: Asynchronously hash default hardcoded memory users as a fallback mechanism
...
try { initDefaultUsers().catch(err => console.error("Error hashing memory users:", err)); } catch (e: any) { console.warn("initDefaultUsers call failed:", e?.message); }
...
// 2D ÔÇö Add Rate Limiting on Login Endpoint: Guard against dictionary and brute-force attacks
...
-- 6. Create about settings mapping
...
-- 7. Create centralized Logging trails table
...
-- 8. Create system settings (dropdown lookups)
...
VALUES
    ('q-1', 'Where did you hear about this career opportunity?', 'select', '["Social Media","School Job Fair","Employee Referral","Newspaper/Flyer","Walk-in"]'::jsonb, true, true, 1),
    ('q-2', 'Are you willing to be assigned to any branch or field office matching CARD MRI priorities?', 'boolean', '[]'::jsonb, true, true, 2),
    ('q-3', 'Are you related to any active employee of CARD MRI entities up to the third degree of consanguinity or affinity?', 'boolean', '[]'::jsonb, true, true, 3),
    ('q-4', 'Do you have experience in field-based operations, collection, or community service work?', 'boolean', '[]'::jsonb, true, true, 4)
ON CONFLICT (id) DO NOTHING;

-- Seed system settings lookups
...
VALUES
    ('job-bmf-001', 'Branch Microfinance Officer', 'Branch Operations', 'CARD Bank, Inc.', 'San Pablo City, Laguna',
     ...
     'Full-time', true),
    ('job-fin-001', 'Accountant / Finance Specialist', 'Finance Center', 'CARD SME Bank, Inc.', 'Lucena City, Quezon',
     ...
     'Full-time', true),
    ('job-it-001', 'IT Support & System Specialist', 'Information Technology Unit', 'CARD MRI IT Mutual Benefit Association', 'Bay, Laguna',
     ...
     'Full-time', true),
    ('job-hr-001', 'Recruitment Coordinator & HR Generalist', 'Human Resource Development', 'CARD Mutually Reinforcing Institutions', 'San Pablo City, Laguna',
...
app.get("/api/setup-database", async (req, res) => {
  ...
  res.json({
    ...
    message: result.success 
      ? "Ô£à CARD MRI Database fully initialized and verified" 
      : "ÔÜá´©Å Setup completed with some warnings",
    timestamp: new Date().toISOString(),
    logs,
    results: result.results,
    note: !process.env.SUPABASE_SERVICE_KEY 
      ? "ÔÜá´©Å SUPABASE_SERVICE_KEY not set. DDL operations may fail. Add it in Vercel environment variables." 
      : "Ô£à Service key detected"
  });
});
